...
// --- 版本資訊 ---
const VERSION = 'v11.18.21 - 黃金比例適配版 (Golden Ratio Fit)'; 
    ...
    return (
        <div className="fixed inset-0 bg-[#F0F8FF] flex items-center justify-center z-[10000]">
            <div className="bg-white p-10 rounded-2xl shadow-xl w-full max-w-md text-center border border-blue-100">
                ...
                <p className="text-gray-400 text-xl mb-8 font-medium">請輸入密碼以存取資料</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <input
                            ...
                            placeholder="請輸入密碼"
                            className="w-full px-4 py-4 text-2xl text-center border-2 border-blue-200 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all placeholder-gray-300 text-gray-700"
                            autoFocus
                            disabled={loadingSettings}
                        />
                    </div>
                    {errorMsg && (
                        <p className="text-red-500 text-xl font-bold animate-pulse">{errorMsg}</p>
                    )}
                    <button
                        type="submit"
                        disabled={loadingSettings}
                        className={`w-full py-4 rounded-xl text-white text-2xl font-bold tracking-wider shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[11000] p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg transform transition-all scale-100 border-4 border-gray-100">
                ...
                <p className="text-gray-500 text-xl mb-6">修改後，所有使用者下次登入皆需使用新密碼。</p>
                
                <div className="space-y-6">
                    <div>
                        <label className="block text-xl font-bold text-gray-700 mb-2">一般模式密碼 (User)</label>
                        <input 
                            type="text" 
                            value={userPwd}
                            onChange={(e) => setUserPwd(e.target.value)}
                            className="w-full p-4 border-2 border-gray-300 rounded-xl text-2xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none"
                        />
                    </div>
                    <div>
                        <label className="block text-xl font-bold text-gray-700 mb-2">管理員密碼 (Admin)</label>
                        <input 
                            type="text" 
                            value={adminPwd}
                ...
                <div className="flex gap-4 mt-8">
                    <button onClick={onClose} className="flex-1 py-3 bg-gray-200 text-gray-700 rounded-xl text-2xl font-bold hover:bg-gray-300 transition">取消</button>
                    <button 
                        onClick={handleSave} 
                        disabled={saving}
                        className="flex-1 py-3 bg-blue-600 text-white rounded-xl text-2xl font-bold hover:bg-blue-700 transition shadow-md"
                    >
                        {saving ? '儲存中...' : '確認修改'}
                    </button>
                </div>
                {isOffline && <p className="mt-4 text-center text-red-500 font-medium text-lg">目前為離線模式，修改不會儲存到雲端。</p>}
            </div>
        </div>
    );
};
                    ...
                    <div className="min-w-full inline-block align-middle">
                        <table className="w-full divide-y divide-gray-300">
                            <thead className="bg-gray-100 sticky top-0 z-50"><tr>
                                    <th className="px-4 py-4 text-2xl font-semibold uppercase tracking-wider text-gray-600 border-r border-gray-300 w-36 sticky left-0 top-0 bg-gray-100 z-50 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">座號</th>
                                    <th className="px-4 py-4 text-2xl font-semibold uppercase tracking-wider text-gray-600 w-48 sticky left-36 top-0 bg-gray-100 z-50 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">姓名</th>
                                ...
                                {(focusedStudentId ? STUDENT_LIST.filter(s => s.id === focusedStudentId) : STUDENT_LIST).map((student) => (
                                    <tr key={student.id} className={`transition duration-100 group ${focusedStudentId ? 'bg-blue-100' : 'hover:bg-blue-50'}`}>
                                        <td 
                                            ...
                                            className="px-4 py-4 text-2xl whitespace-nowrap font-medium text-gray-900 border-r border-gray-300 w-36 sticky left-0 bg-white z-30 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] cursor-pointer group-hover:text-blue-600 group-hover:bg-blue-100"
                                        ...
                                        <td 
                                            onClick={() => setFocusedStudentId(focusedStudentId === student.id ? null : student.id)}
                                            className="px-4 py-4 text-2xl whitespace-nowrap text-gray-900 font-semibold w-48 sticky left-36 bg-white z-30 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] cursor-pointer group-hover:text-blue-600 group-hover:bg-blue-100 flex items-center justify-center gap-2"
                                            title={focusedStudentId === student.id ? "點擊以顯示全部學生" : "點擊以只顯示此學生"}
                                        >
                                            {student.name}
