...
// --- 版本資訊 ---
const VERSION = 'v11.18.20 - 強制滿版與防斷行 (Full Width & No Wrap)'; 
            ...
            <div className="w-full relative overflow-x-auto border border-gray-300 rounded-lg shadow-lg">
                <table className="min-w-full divide-y divide-gray-300">
                    <thead className="bg-gray-200 sticky top-0 z-30">
                        <tr>
                            <th className="px-4 py-4 text-2xl font-semibold uppercase tracking-wider text-gray-700 w-36 sticky left-0 bg-gray-200 z-40 border-r border-gray-300 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">姓名</th>
                            {months.map(month => (
                                <th key={month.id} className={`px-4 py-4 text-2xl font-semibold uppercase tracking-wider text-white min-w-[200px] ${month.color}`}>{month.name}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                        {studentIds.map(studentId => {
                            ...
                            return (
                                <tr key={studentId} className="hover:bg-gray-50 transition duration-100">
                                    <td className="px-4 py-4 text-2xl whitespace-nowrap text-gray-900 font-semibold w-36 sticky left-0 bg-white z-10 border-r border-gray-300 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">{studentData.studentName}</td>
                                    {months.map(month => {
                                        ...
                                        return (
                                            <td key={month.id} className={`px-4 py-4 whitespace-nowrap text-center text-2xl ${hasMissing ? 'bg-red-100' : (hasLate ? 'bg-yellow-100' : (hasCompletedOnly ? 'bg-green-100' : 'bg-white'))}`}>
                                                {hasTotal ? (
                                                    <div className="flex flex-col sm:flex-row justify-center items-center gap-1">
                                                        <span className="text-green-700 whitespace-nowrap text-lg">完:{stats.daysCompleted}</span>
                                                        <span className={`${hasLate ? 'font-bold text-yellow-600' : 'text-gray-700'} whitespace-nowrap text-lg`}>遲:{stats.daysLate}</span>
                                                        <span className={`${hasMissing ? 'font-bold text-red-600' : 'text-gray-700'} whitespace-nowrap text-lg`}>缺:{stats.daysMissing}</span>
                                                    </div>
                                ...
                                {columnItems.map((item) => {
                                    ...
                                    return (
                                        <li key={item.assignmentId} className={`p-3 flex items-center gap-3 text-2xl text-gray-700 cursor-pointer transition duration-100 ${isSelected ? 'bg-blue-200' : 'hover:bg-blue-50'}`} onClick={() => handleToggleSelect(item.assignmentId)}>
                                            ...
                                            <span className="font-medium text-gray-900 w-28">{item.date}</span>
...
const AssignmentHeader = ({ assignment, isGlobalLoading, handleDeleteAssignment, handleEditSave, handleMoveAssignment, setEditingAssignmentId, setEditingAssignmentName, editingAssignmentId, editingAssignmentName }) => {
    ...
    return (
        <th ref={(node) => drag(drop(node))} style={{ opacity: isDragging ? 0.4 : 1, cursor: isGlobalLoading ? 'default' : 'grab' }} className={`px-4 py-4 text-2xl text-center font-semibold uppercase tracking-wider text-gray-800 transition duration-100 ease-in-out sticky top-0 z-50 bg-gray-100`}>
                    ...
                    <div className="min-w-full inline-block align-middle">
                        <table className="w-full divide-y divide-gray-300">
                            <thead className="bg-gray-100 sticky top-0 z-50"><tr>
                                    ...
                                    <th className="px-4 py-4 text-2xl font-semibold uppercase tracking-wider text-gray-600 w-32 sticky left-28 top-0 bg-gray-100 z-50 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">姓名</th>
                            ...
                            <tbody className={`divide-y divide-gray-200 ${focusedStudentId ? 'bg-blue-50' : 'bg-white'}`}>
                                {(focusedStudentId ? STUDENT_LIST.filter(s => s.id === focusedStudentId) : STUDENT_LIST).map((student) => (
                                    <tr key={student.id} className={`transition duration-100 group ${focusedStudentId ? 'bg-blue-100' : 'hover:bg-blue-50'}`}>
                                        ...
                                        <td 
                                            onClick={() => setFocusedStudentId(focusedStudentId === student.id ? null : student.id)}
                                            className="px-4 py-4 text-2xl whitespace-nowrap text-gray-900 font-semibold w-32 sticky left-28 bg-white z-30 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] cursor-pointer group-hover:text-blue-600 group-hover:bg-blue-100 flex items-center justify-center gap-2"
                                            title={focusedStudentId === student.id ? "點擊以顯示全部學生" : "點擊以只顯示此學生"}
                                        >
                                            {student.name}
