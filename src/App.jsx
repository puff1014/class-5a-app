...
// --- 版本資訊 ---
const VERSION = 'v20.0.19 - 橫向佈局與表格修復版'; 
...
// --- 學生學習歷程 Dashboard Modal (V20.0.19: 文字全面加大兩號 + 圖表增高至600px) ---
    ...
    return (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-80 flex items-center justify-center z-[99999] p-4 backdrop-blur-sm animate-fade-in">
            ...
            <div className={`bg-white rounded-3xl shadow-2xl w-full max-w-[98vw] h-[98vh] flex flex-col overflow-hidden border-[8px] ${currentFeedback.isAlert ? 'border-red-500' : 'border-white'}`}>
                ...
                {/* 1. Header 區域 (V20.0.19: 文字加大兩號) */}
                <div className={`px-4 py-2 flex justify-between items-center text-white shrink-0 transition-colors duration-500 ${currentFeedback.isAlert ? 'bg-red-600' : (viewMode === 'TREND' ? 'bg-gradient-to-r from-blue-600 to-cyan-500' : 'bg-gradient-to-r from-indigo-600 to-purple-500')}`}>
                    <div className="flex items-center gap-4 w-full">
                        {/* 左側：座號 + 姓名 + 標題 */}
                        <div className="flex items-center gap-4 shrink-0">
                            {/* 座號加大至 6xl */}
                            <div className={`w-24 h-24 bg-white rounded-full flex items-center justify-center text-6xl font-bold shadow-lg border-4 ${currentFeedback.isAlert ? 'text-red-600 border-red-200' : (viewMode === 'TREND' ? 'text-blue-600 border-blue-200' : 'text-indigo-600 border-indigo-200')}`}>{student.id}</div>
                            <div className="flex flex-col justify-center">
                                {/* 標題加大至 7xl */}
                                <h2 className="text-7xl font-black tracking-wide leading-none mb-1">{maskedName} 的學習歷程</h2>
                                {/* 副標題加大至 3xl */}
                                <p className="text-white/90 text-3xl font-medium flex items-center gap-2"><Activity className="w-6 h-6" /> {semesterId === 'S1' ? '上學期' : '下學期'}綜合分析報表</p>
                            </div>
                        </div>

                        {/* 分隔線 */}
                        <div className="h-16 w-[2px] bg-white/30 mx-2"></div>

                        {/* 右側：分數 + 獅王 + 評語 */}
                        <div className="flex flex-1 items-center bg-white/10 rounded-xl px-5 py-3 backdrop-blur-sm border border-white/20 h-full">
                            <div className="flex flex-col shrink-0 mr-6">
                                {/* 標籤加大至 2xl */}
                                <div className="text-2xl font-bold text-white/80 mb-0">🏆 綜合總分</div>
                                {/* 分數加大至 7xl */}
                                <span className="text-7xl font-black text-yellow-300 drop-shadow-md leading-none">{overallData.score}</span>
                            </div>
                            <div className="h-14 w-[2px] bg-white/30 mr-6"></div>
                            <div className="flex items-center flex-1">
                                {/* 動物加大至 6xl */}
                                <span className="text-6xl font-bold text-white mr-4 shrink-0">{overallBadge.animal}</span>
                                {/* 評語加大至 5xl */}
                                <div className="text-5xl font-medium text-white/95 leading-tight overflow-hidden text-ellipsis whitespace-nowrap bg-white/20 px-4 py-2 rounded-xl backdrop-blur-md border border-white/30">{overallBadge.comment}</div>
                            </div>
                        </div>
                    </div>
                    <button onClick={onClose} className="bg-white/20 hover:bg-white/30 p-3 rounded-full transition backdrop-blur-md ml-3 shrink-0"><X className="w-8 h-8" /></button>
                </div>

                {/* 2. 工具列 (按鈕 + 資產顯示) - 文字加大兩號 */}
                <div className="bg-gray-100 p-4 flex justify-between items-center shadow-inner shrink-0">
                    <div className="flex gap-4">
                        {/* 按鈕加大至 4xl */}
                        <button onClick={() => setViewMode('STATUS')} className={`px-8 py-4 rounded-xl text-4xl font-bold transition-all duration-300 flex items-center gap-3 ${viewMode === 'STATUS' ? 'bg-white text-indigo-600 shadow-md ring-2 ring-indigo-200' : 'text-gray-500 hover:bg-gray-200'}`}><BarChart2 className="w-10 h-10"/> 狀況統計</button>
                        <button onClick={() => setViewMode('TREND')} className={`px-8 py-4 rounded-xl text-4xl font-bold transition-all duration-300 flex items-center gap-3 ${viewMode === 'TREND' ? 'bg-white text-blue-600 shadow-md ring-2 ring-blue-200' : 'text-gray-500 hover:bg-gray-200'}`}><TrendingUp className="w-10 h-10"/> 績效分數</button>
                    </div>
                    <div className="bg-white px-8 py-3 rounded-xl shadow-sm border border-gray-200 flex items-center gap-6">
                        {/* 標籤加大至 3xl */}
                        <span className="text-gray-500 text-3xl font-bold flex items-center gap-3"><Coins className="w-10 h-10 text-yellow-500"/> 目前資產:</span>
                        <div className="flex items-baseline gap-3">
                            {/* 數字加大至 6xl, 單位加大至 3xl */}
                            <span className="text-6xl font-black text-gray-800">{bankBalance?.gold || 0}</span><span className="text-3xl text-yellow-500 font-bold">金</span>
                            <span className="text-5xl text-gray-300 font-light">/</span>
                            <span className="text-6xl font-black text-gray-800">{bankBalance?.silver || 0}</span><span className="text-3xl text-gray-400 font-bold">銀</span>
                        </div>
                    </div>
                </div>

                {/* 3. 主要內容區 */}
                <div className={`flex-1 overflow-auto p-6 ${currentFeedback.bg} flex flex-col gap-6`}>
                    
                    {/* 上半部：資訊圖卡 - 文字加大兩號 */}
                    <div className="flex gap-6 mb-2 shrink-0">
                        {/* 左卡：本學期統計 */}
                        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 flex flex-col justify-center items-center relative overflow-hidden shrink">
                            <div className="absolute top-0 left-0 w-full h-4 bg-gradient-to-r from-blue-400 to-indigo-500"></div>
                            
                            {/* 標題加大至 5xl */}
                            <p className="text-gray-500 text-5xl font-bold mb-4">本學期{viewMode === 'STATUS' ? '統計天數' : '作業總數'}</p>
                            
                            <div className="flex items-end gap-6 justify-center">
                                {/* 巨大數字加大至 8xl */}
                                <div className="flex items-baseline leading-none">
                                    <span className={`text-8xl font-black ${viewMode === 'STATUS' ? 'text-indigo-600' : 'text-blue-600'}`}>{currentStats.total}</span>
                                    <span className="text-5xl text-gray-400 font-bold ml-2">{statsUnit}</span>
                                </div>

                                {/* 小數據方塊加大至 6xl */}
                                <div className="flex gap-4">
                                    <div className="flex flex-col items-center bg-green-50 px-4 py-2 rounded-xl border border-green-100">
                                        <span className="text-6xl font-bold text-green-600 leading-none">{currentStats.completed}</span>
                                        <span className="text-2xl text-green-800 font-bold">準時</span>
                                    </div>
                                    <div className="flex flex-col items-center bg-yellow-50 px-4 py-2 rounded-xl border border-yellow-100">
                                        <span className="text-6xl font-bold text-yellow-600 leading-none">{currentStats.late}</span>
                                        <span className="text-2xl text-yellow-800 font-bold">遲交</span>
                                    </div>
                                    <div className="flex flex-col items-center bg-red-50 px-4 py-2 rounded-xl border border-red-100">
                                        <span className="text-6xl font-bold text-red-600 leading-none">{currentStats.missing}</span>
                                        <span className="text-2xl text-red-800 font-bold">缺交</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* 右卡：分數分析 */}
                        <div className={`p-6 rounded-[2rem] shadow-sm border-l-[12px] flex items-center justify-between grow ${currentFeedback.border} ${currentFeedback.bg}`}>
                            <div className="flex flex-col items-center gap-0 pl-4 shrink-0">
                                {/* 標籤加大至 4xl */}
                                <span className="text-4xl font-bold text-gray-400 mb-2">健康指數</span>
                                <div className="flex items-baseline">
                                    {/* 分數加大至 8xl */}
                                    <span className={`text-8xl font-black leading-none ${currentFeedback.color}`}>{viewMode === 'STATUS' ? summaryStats.avgScore : trendStats.avgScore}</span>
                                    <span className="text-4xl text-gray-400 font-bold ml-3">分</span>
                                </div>
                            </div>
                            
                            <div className="flex-1 pl-8 border-l-4 border-gray-200/50 ml-8">
                                {/* 評語加大至 6xl */}
                                <p className={`${currentFeedback.color} font-bold text-6xl leading-tight text-left`}>{currentFeedback.text}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-6">
                        {/* 圖表區 (V20.0.19: 高度加大至 600px) */}
                        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-200 h-[600px] shrink-0">
                            <div className="flex items-center justify-between mb-4">
                                {/* 標題加大至 5xl */}
                                <h3 className="text-5xl font-bold text-gray-700 flex items-center">{viewMode === 'TREND' ? (<><TrendingUp className="w-12 h-12 mr-4 text-blue-500" /> 作業績效趨勢圖</>) : (<><BarChart2 className="w-12 h-12 mr-4 text-indigo-500" /> 每月作業狀況分佈</>)}</h3>
                            </div>
                            {/* 內部圖表高度設為 520px */}
                            <div className="w-full h-[520px]">{viewMode === 'TREND' ? ( <SimpleLineChart data={trendData} height={520} /> ) : ( <SimpleStackedBarChart data={healthData} height={520} /> )}</div>
                        </div>

                        {/* 表格區 - 文字全面加大至 5xl */}
                        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-200 overflow-hidden">
                            <div className="p-6 bg-gray-50 border-b border-gray-200 text-5xl font-bold text-gray-600">詳細數據列表</div>
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-white"><tr><th className="px-8 py-6 text-left text-5xl font-bold text-gray-600">月份</th><th className="px-8 py-6 text-center text-5xl font-bold text-green-600">準時</th><th className="px-8 py-6 text-center text-5xl font-bold text-yellow-600">補交</th><th className="px-8 py-6 text-center text-5xl font-bold text-red-600">缺交</th><th className="px-8 py-6 text-center text-5xl font-bold text-blue-600">{viewMode === 'STATUS' ? '健康平均' : '績效平均'}</th></tr></thead>
                                    <tbody className="divide-y divide-gray-200">
                                        {chartData.map((row, idx) => {
                                            const tVal = row.value || 0;
                                            return (
                                                <tr key={idx} className="hover:bg-gray-50"><td className="px-8 py-6 text-5xl font-bold text-gray-800">{row.label}</td><td className="px-8 py-6 text-center text-5xl font-medium text-gray-600">{row.details.onTime}</td><td className="px-8 py-6 text-center text-5xl font-medium text-gray-600">{row.details.late}</td><td className="px-8 py-6 text-center text-5xl font-medium text-gray-600">{row.details.missing}</td><td className="px-8 py-6 text-center"><span className={`inline-block px-6 py-3 rounded-full text-5xl font-bold ${tVal >= 90 ? 'bg-green-100 text-green-700' : (tVal >= 60 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700')}`}>{tVal.toFixed(1)}</span></td></tr>
                                            );
                                        })}
                                    </tbody>
                                </table>
