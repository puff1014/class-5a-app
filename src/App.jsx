...
// --- 版本資訊 ---
const VERSION = 'v11.18.17 - 完美適配版 (Perfect Fit - No Cutoff)'; 
...
const CustomAlert = ({ message, onClose }) => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4">
        <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md transform transition-all duration-300 scale-100">
            <h3 className="text-2xl font-semibold text-gray-800 mb-4">通知</h3>
            <p className="text-lg text-gray-600 mb-6">{message}</p>
            <button
                onClick={onClose}
                className="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition duration-150 ease-in-out font-medium text-xl"
            >
                確定
            </button>
        </div>
...
const LoginScreen = ({ onLogin, loadingSettings, errorMsg }) => {
    ...
    return (
        <div className="fixed inset-0 bg-[#F0F8FF] flex items-center justify-center z-[10000]">
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm text-center border border-blue-100">
                <div className="flex justify-center mb-6">
                    <div className="p-3 rounded-2xl border-2 border-blue-400">
                        <Lock className="w-10 h-10 text-blue-500" />
                    </div>
                </div>
                <h1 className="text-3xl font-bold text-gray-800 mb-2 tracking-wide">五年甲班作業表</h1>
                <p className="text-gray-400 text-lg mb-8 font-medium">請輸入密碼以存取資料</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <input
                            ...
                            placeholder="請輸入密碼"
                            className="w-full px-4 py-3 text-2xl text-center border-2 border-blue-200 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all placeholder-gray-300 text-gray-700"
                            autoFocus
                            disabled={loadingSettings}
                        />
                    </div>
                    {errorMsg && (
                        <p className="text-red-500 text-lg font-bold animate-pulse">{errorMsg}</p>
                    )}
                    <button
                        type="submit"
                        disabled={loadingSettings}
                        className={`w-full py-3 rounded-xl text-white text-2xl font-bold tracking-wider shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2
                            ${loadingSettings ? 'bg-gray-400 cursor-wait' : 'bg-blue-500 hover:bg-blue-600'}
                        `}
                    >
                        {loadingSettings ? '讀取設定中...' : <><Lock className="w-5 h-5" /> 解鎖</>}
                    </button>
                </form>
...
const PasswordSettingsModal = ({ currentSettings, onSave, onClose, isOffline }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[11000] p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md transform transition-all scale-100 border-4 border-gray-100">
                <h3 className="text-2xl font-bold text-gray-800 mb-2 flex items-center gap-3">
                    <Settings className="w-6 h-6 text-gray-600" />
                    系統密碼設定
                </h3>
                <p className="text-gray-500 text-base mb-6">修改後，所有使用者下次登入皆需使用新密碼。</p>
                
                <div className="space-y-4">
                    <div>
                        <label className="block text-lg font-bold text-gray-700 mb-1">一般模式密碼 (User)</label>
                        <input 
                            type="text" 
                            value={userPwd}
                            onChange={(e) => setUserPwd(e.target.value)}
                            className="w-full p-3 border-2 border-gray-300 rounded-xl text-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none"
                        />
                    </div>
                    <div>
                        <label className="block text-lg font-bold text-gray-700 mb-1">管理員密碼 (Admin)</label>
                        <input 
                            type="text" 
                            value={adminPwd}
                ...
                <div className="flex gap-4 mt-8">
                    <button onClick={onClose} className="flex-1 py-2 bg-gray-200 text-gray-700 rounded-xl text-xl font-bold hover:bg-gray-300 transition">取消</button>
                    <button 
                        onClick={handleSave} 
                        disabled={saving}
                        className="flex-1 py-2 bg-blue-600 text-white rounded-xl text-xl font-bold hover:bg-blue-700 transition shadow-md"
                    >
                        {saving ? '儲存中...' : '確認修改'}
                    </button>
                </div>
                {isOffline && <p className="mt-4 text-center text-red-500 font-medium text-base">目前為離線模式，修改不會儲存到雲端。</p>}
            </div>
        </div>
    );
};
...
const AllMissingAssignmentsModal = ({ missingStats, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[10000] p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-4xl h-[85vh] flex flex-col border border-gray-200">
                ...
                <div className="flex-1 overflow-auto">
                    {studentsWithMissing.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-gray-400">
                            <Check className="w-20 h-20 mb-4 text-green-400" />
                            <p className="text-2xl font-bold text-green-600">太棒了！目前全班皆已完成所有作業。</p>
                        </div>
                    ) : (
                        <table className="min-w-full divide-y divide-gray-300">
                            <thead className="bg-gray-100 sticky top-0 z-10">
                                <tr>
                                    <th className="px-3 py-3 text-lg font-bold text-gray-700 uppercase tracking-wider w-20 text-center border-r border-gray-300">座號</th>
                                    <th className="px-3 py-3 text-lg font-bold text-gray-700 uppercase tracking-wider w-28 text-center border-r border-gray-300">姓名</th>
                                    <th className="px-3 py-3 text-lg font-bold text-gray-700 uppercase tracking-wider w-24 text-center border-r border-gray-300">缺交數</th>
                                    <th className="px-4 py-3 text-lg font-bold text-gray-700 uppercase tracking-wider text-left">未完成項目明細</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-gray-200">
                                {studentsWithMissing.map((student) => (
                                    <tr key={student.id} className="hover:bg-red-50 transition duration-100">
                                        <td className="px-3 py-3 text-lg text-gray-900 font-medium text-center border-r border-gray-200">{student.id}</td>
                                        <td className="px-3 py-3 text-lg text-gray-900 font-bold text-center border-r border-gray-200">{student.name}</td>
                                        <td className="px-3 py-3 text-center border-r border-gray-200">
                                            <span className="inline-flex items-center justify-center px-3 py-1 rounded-full bg-red-100 text-red-800 font-bold text-lg">
                                            ...
                                            <ul className="list-disc list-inside space-y-1">
                                                {[...student.missingDetails]
                                                    ...
                                                    <li key={idx} className="flex items-start">
                                                        <span className="text-red-600 font-bold text-base mr-2">{detail.assignment}</span>
                                                        <span className="font-mono font-medium text-gray-400 text-sm">[{new Date(detail.date).toLocaleDateString('zh-TW', {month:'numeric', day:'numeric'})}]</span>
                                                    </li>
                                                ))}
                                            </ul>
                                        </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[9999] p-4">
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md transform transition-all duration-300 scale-100">
                ...
                <div className="flex justify-between gap-4 mt-6">
                    <button onClick={onCancel} className="flex-1 bg-gray-300 text-gray-800 py-2 rounded-lg hover:bg-gray-400 transition duration-150 ease-in-out font-medium text-xl">取消</button>
                    <button
                        onClick={() => {
                            if (isAltPressed) { onConfirm(); } else { alert(`請按住 Alt 鍵，才能確認執行 ${confirmTitle} 操作！`); }
                        }}
                        disabled={!isAltPressed}
                        className={`flex-1 text-white py-2 rounded-lg transition duration-150 ease-in-out font-medium text-xl ${confirmColor} ${isAltPressed ? 'hover:brightness-110' : 'bg-red-400 cursor-not-allowed'}`}
                    >
                        {confirmTitle} 
                    </button>
                </div>
                <p className="mt-3 text-center text-red-500 text-lg font-semibold opacity-0">請按住 **Alt 鍵** 才能啟用刪除按鈕！</p>
            </div>
        </div>
    );
};
    ...
    return (
        <div className="mt-6 p-4 bg-white rounded-xl shadow-xl border border-gray-200">
            <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center">
                <span className="text-pink-500 text-2xl mr-2">🎨</span>顏色分級說明
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-10 gap-2"> 
                {legendTiers.map((item, index) => (
                    <div key={index} className={`
                        py-2 px-1 rounded-lg text-center cursor-default
                        ${item.classes.bg} ${item.classes.border} 
                        border-2 border-b-4
                        flex items-center justify-center
                    `}>
                        <p className={`text-base font-bold ${item.classes.text} leading-tight`}>{item.count}</p>
                    </div>
                ))}
            </div>
        </div>
    ...
    return (
        <div className="mt-8 p-4 bg-white rounded-xl shadow-xl border border-gray-200 max-w-full">
            <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center">
                <span className="text-2xl mr-2">📊</span><span className="text-xl">每月繳交狀況統計</span>
            </h2> 
            <div className="w-full relative overflow-x-auto border border-gray-300 rounded-lg shadow-lg">
                <table className="min-w-full divide-y divide-gray-300">
                    <thead className="bg-gray-200 sticky top-0 z-30">
                        <tr>
                            <th className="px-3 py-3 text-lg font-semibold uppercase tracking-wider text-gray-700 w-24 sticky left-0 bg-gray-200 z-40 border-r border-gray-300 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">姓名</th>
                            {months.map(month => (
                                <th key={month.id} className={`px-3 py-3 text-lg font-semibold uppercase tracking-wider text-white min-w-[120px] ${month.color}`}>{month.name}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                        {studentIds.map(studentId => {
                            ...
                            return (
                                <tr key={studentId} className="hover:bg-gray-50 transition duration-100">
                                    <td className="px-3 py-3 text-lg whitespace-nowrap text-gray-900 font-semibold w-24 sticky left-0 bg-white z-10 border-r border-gray-300 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">{studentData.studentName}</td>
                                    {months.map(month => {
                                        ...
                                        return (
                                            <td key={month.id} className={`px-3 py-3 whitespace-nowrap text-center text-lg ${hasMissing ? 'bg-red-100' : (hasLate ? 'bg-yellow-100' : (hasCompletedOnly ? 'bg-green-100' : 'bg-white'))}`}>
                                                {hasTotal ? (
                                                    <div className="flex flex-col sm:flex-row justify-center items-center gap-1">
                                                        <span className="text-green-700 whitespace-nowrap text-sm">完:{stats.daysCompleted}</span>
                                                        <span className={`${hasLate ? 'font-bold text-yellow-600' : 'text-gray-700'} whitespace-nowrap text-sm`}>遲:{stats.daysLate}</span>
                                                        <span className={`${hasMissing ? 'font-bold text-red-600' : 'text-gray-700'} whitespace-nowrap text-sm`}>缺:{stats.daysMissing}</span>
                                                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-2">
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full transform transition-all duration-300 scale-100 max-h-[95vh] flex flex-col">
                ...
                <div className={`p-4 rounded-xl mb-4 shadow-md border-l-8 ${colorClasses.bg} ${colorClasses.border} text-center`}>
                    <div className={`text-2xl font-semibold ${colorClasses.text}`}>累積總計：<span className={`ml-2 font-black ${colorClasses.countText} text-3xl`}>{missingCount}</span> 次</div>
                </div>
                <div className="flex justify-between items-center mb-2 border-b pb-2">
                    <h4 className="text-xl font-bold text-gray-800">詳細未訂正項目 ({detailedMissingItems.length} 筆紀錄):</h4>
                    <button onClick={handleToggleSelectAll} className="text-lg font-medium text-blue-600 hover:text-blue-800 transition">{selectedItemIds.length === detailedMissingItems.length ? '取消全選' : '全選'}</button>
                </div>
                        ...
                        {columns.map((columnItems, colIndex) => (
                            <ul key={colIndex} className={`divide-y divide-gray-200 rounded-lg ${colIndex % 2 === 0 ? 'bg-white' : 'bg-gray-100'}`}>
                                {columnItems.map((item) => {
                                    ...
                                    return (
                                        <li key={item.assignmentId} className={`p-3 flex items-center gap-3 text-lg text-gray-700 cursor-pointer transition duration-100 ${isSelected ? 'bg-blue-200' : 'hover:bg-blue-50'}`} onClick={() => handleToggleSelect(item.assignmentId)}>
                                            <input className="h-5 w-5 text-blue-600 rounded cursor-pointer" onClick={(e) => e.stopPropagation()} />
                                            <span className="font-medium text-gray-900 w-24">{item.date}</span>
                ...
                <div className="mt-4 pt-3 border-t border-green-300">
                    <button onClick={handleBatchDeleteSelectedItems} disabled={selectedItemIds.length === 0} className={`w-full py-3 rounded-lg transition duration-150 ease-in-out font-medium text-xl flex items-center justify-center shadow-lg ${selectedItemIds.length === 0 ? 'bg-gray-400 cursor-not-allowed text-gray-200' : 'bg-green-600 hover:bg-green-700 text-white'}`} title="按住 Control (Ctrl/Cmd) 鍵並點擊以將選定的項目標記為已補交 (遲繳)">
                        <span className="text-2xl mr-2">⚠️</span> 批次標記 {selectedItemIds.length} 項為「已補交 (遲繳)」
                    </button>
                </div>
                <button onClick={onClose} className="mt-4 w-full bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 transition duration-150 ease-in-out font-medium text-xl">關閉</button>
            </div>
        </div>
    );
};
...
const AssignmentHeader = ({ assignment, isGlobalLoading, handleDeleteAssignment, handleEditSave, handleMoveAssignment, setEditingAssignmentId, setEditingAssignmentName, editingAssignmentId, editingAssignmentName }) => {
    ...
    return (
        <th ref={(node) => drag(drop(node))} style={{ opacity: isDragging ? 0.4 : 1, cursor: isGlobalLoading ? 'default' : 'grab' }} className={`px-2 py-3 text-xl text-center font-semibold uppercase tracking-wider text-gray-800 transition duration-100 ease-in-out sticky top-0 z-50 bg-gray-100`}>
            <div className="flex flex-col items-center justify-center min-w-[80px] group relative">
                <div className={`relative p-2 rounded-lg shadow-sm transition duration-100 border-2 border-transparent ${isEditing ? 'ring-2 ring-blue-400 bg-white' : 'hover:bg-gray-50 bg-white'}`} onDoubleClick={handleEditStart}>
                    {isEditing ? (
                        <input type="text" value={editingAssignmentName} onChange={(e) => setEditingAssignmentName(e.target.value)} onBlur={handleLocalEditSave} onKeyDown={(e) => { if (e.key === 'Enter') { e.target.blur(); } else if (e.key === 'Escape') { setEditingAssignmentId(null); setEditingAssignmentName(''); } }} className="font-bold text-center text-xl w-full focus:outline-none bg-transparent" autoFocus disabled={isGlobalLoading} />
...
const DateTab = ({ date, isSelected, onClick }) => {
    ...
    return (
        <button onClick={() => onClick(date)} className={`px-4 py-2 text-xl font-semibold rounded-lg transition duration-150 ease-in-out shadow-md whitespace-nowrap ${isSelected ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}>
            {formattedDate}
        </button>
    );
};
                                ...
                                {(focusedStudentId ? STUDENT_LIST.filter(s => s.id === focusedStudentId) : STUDENT_LIST).map((student) => (
                                    <tr key={student.id} className={`transition duration-100 group ${focusedStudentId ? 'bg-blue-100' : 'hover:bg-blue-50'}`}>
                                        <td 
                                            ...
                                            className="px-4 py-4 text-2xl whitespace-nowrap font-medium text-gray-900 border-r border-gray-300 w-28 sticky left-0 bg-white z-30 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] cursor-pointer group-hover:text-blue-600 group-hover:bg-blue-100"
                                        ...
                                        <td 
                                            onClick={() => setFocusedStudentId(focusedStudentId === student.id ? null : student.id)}
                                            className="px-4 py-4 text-2xl whitespace-nowrap text-gray-900 font-semibold w-40 sticky left-28 bg-white z-30 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] cursor-pointer group-hover:text-blue-600 group-hover:bg-blue-100 flex items-center justify-center gap-2"
                                            title={focusedStudentId === student.id ? "點擊以顯示全部學生" : "點擊以只顯示此學生"}
                                        >
                                            {student.name}
